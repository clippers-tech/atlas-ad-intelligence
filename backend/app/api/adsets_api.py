...
def _merge_breakdowns(rows: list) -> list[dict]:
    """Merge conversion_breakdown JSON across metrics."""
...
async def _adset_metrics(
    ...
    """Aggregate ad_metrics for an ad set."""
    empty = {
    ...
    # Aggregate conversion breakdowns
    bd_q = select(
    ...
    return {
        ...
        "cost_per_lpv": (
            round(s / lpv, 2) if lpv else 0.0
        ),
        "cpl": cpr,
        "cost_per_result": cpr,
        "frequency": (
            round(imp / rch, 2) if rch else 0.0
        ),
...
async def list_ad_sets(
    ...
    """List ad sets enriched with metrics."""
    q = (
        select(AdSet)
